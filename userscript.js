// ==UserScript==
// @name         U校园全自动助手 (v19.9 - 终极修正版)
// @namespace    http://tampermonkey.net/
// @version      19.9.0
// @description  [终极修正] 彻底修复了v19.8中致命的单元泛化逻辑错误，确保脚本能为所有单元（1-6）正确匹配并执行任务模板。这才是真正可靠的最终版本。
...
(function() {
    ...
    // This GENERIC structure is now correctly applied to all units (1-6).
    const COURSE_DATA = {
    ...
    async function getPageContextAndAnswers() {
        ...
        const unitId = unitTaskMatch[1];
        const taskId = unitTaskMatch[2];
        const unitKey = `Unit ${unitId}`;
        const taskSuffixToFind = `-${taskId}`;
    ...
    async function doAnswer() { if (Object.keys(COURSE_DATA.QUESTION_BANK).length === 0) { log('题库为空...', 'warn'); await sleep(3000); if (Object.keys(COURSE_DATA.QUESTION_BANK).length === 0) { log('题库加载失败', 'error'); return; } } const context = await getPageContextAndAnswers(); if (!context) return; if (context === 'answered') { log('此部分已作答', 'info'); return; } const { answers, mainTaskContent, activeSubTaskName } = context; const choiceQuestions = document.querySelectorAll('.question-common-abs-reply, .question-common-abs-banked-cloze'); const textInputs = document.querySelectorAll('textarea.question-inputbox-input'); const fillInBlanks = document.querySelectorAll('.fe-scoop .comp-abs-input input'); let answered = false; if (choiceQuestions.length > 0 && typeof answers === 'string' && /^[A-Z](?:\s*[A-Z])*$/.test(answers.trim())) { const answerLetters = answers.trim().replace(/\s+/g, ''); for (let i = 0; i < Math.min(choiceQuestions.length, answerLetters.length); i++) { if (choiceQuestions[i].querySelector('.option.selected')) continue; for (const option of choiceQuestions[i].querySelectorAll('.option')) { if (option.querySelector('.caption')?.textContent.trim() === answerLetters[i]) { robustClick(option); await sleep(250); answered = true; break; } } } } else if (textInputs.length > 0 && typeof answers === 'string') { const answerLines = answers.split('\n').map(l => l.replace(/^\d+\.?\)?\s*/, '').trim()).filter(Boolean); for (let i = 0; i < Math.min(textInputs.length, answerLines.length); i++) { if (!textInputs[i].value) { textInputs[i].value = answerLines[i]; textInputs[i].dispatchEvent(new Event('input', { bubbles: true })); await sleep(250); answered = true; } } } else if (fillInBlanks.length > 0 && typeof answers === 'string') { const answerLines = answers.split('\n').map(l => l.replace(/^\d+\.?\)?\s*/, '').trim()).filter(Boolean); for (let i = 0; i < Math.min(fillInBlanks.length, answerLines.length); i++) { if (!fillInBlanks[i].value) { fillInBlanks[i].value = answerLines[i]; fillInBlanks[i].dispatchEvent(new Event('input', { bubbles: true })); await sleep(250); answered = true; } } } if (answered) { log(`应用答案: ${activeSubTaskName}`, "success"); if (activeSubTaskName !== "Part III") { lastAnsweredSubTask = `${mainTaskContent}-${activeSubTaskName}`; } } }
    ...
    async function handleVocabularyTask() { log("处理单词卡...", 'info'); if (!document.querySelector('.vocabulary-wrapper')) return; let maxClicks = 50; while (maxClicks-- > 0) { if (!isAutomationRunning) return; const nextButton = document.querySelector('.vocActions .next:not(.swiper-button-disabled)'); if (!nextButton) { log("单词卡已翻完。", 'success'); return; } robustClick(nextButton); await sleep(700); } log("单词卡点击次数达到上限。", 'warn'); }

    // --- Core Automation Engine: The "Mechanical" Executor ---
    async function main() {
        if (isProcessing || !isAutomationRunning) return;
        isProcessing = true;
        try {
            await sleep(3500);
            const mainTaskNameElement = document.querySelector('.pc-menu-activity .pc-menu-node-name');
            const currentMainTaskName = mainTaskNameElement?.getAttribute('title')?.trim();
            if (!currentMainTaskName) throw new Error("无法识别当前主任务。");

            const lastProcessed = GM_getValue('lastProcessedTaskName', '');
            if (currentMainTaskName === lastProcessed && location.href === GM_getValue('lastUrl', '')) { log(`检测到卡在任务: "${currentMainTaskName}"。强制导航。`, 'warn'); await navigateToNextMainTask(); return; }
            if (currentMainTaskName !== lastProcessed) { lastAnsweredSubTask = ''; }

            const unitTaskMatch = currentMainTaskName.match(/^(\d+)-(\d+)/);
            if (!unitTaskMatch) { log(`无法从任务 "${currentMainTaskName}" 解析出 单元-任务ID，尝试导航。`, 'warn'); await navigateToNextMainTask(); return; }
            const unitId = unitTaskMatch[1];
            const taskId = unitTaskMatch[2];
            const taskSuffixToFind = `-${taskId}`;

            log(`读取 [Unit ${unitId}] 任务地图: "${currentMainTaskName}" (模板后缀: ${taskSuffixToFind})`, 'success');
            const taskData = COURSE_DATA.GENERIC_UNIT_STRUCTURE.find(t => t.id_suffix === taskSuffixToFind);

            if (!taskData) { log(`未在地图中找到后缀为 "${taskSuffixToFind}" 的执行模板，尝试导航。`, 'warn'); await navigateToNextMainTask(); return; }

            if (taskData.type === 'standalone') {
                log(`执行独立任务: ${taskData.name}`);
                await executeActions(taskData.actions);
            } else if (taskData.type === 'composite' && taskData.children) {
                log(`执行复合任务: ${taskData.name}`);
                for (const subTaskConfig of taskData.children) {
                    ...
                    const subTaskElement = subTaskElements.find(el => (el.getAttribute('title') || el.textContent.trim()) === subTaskConfig.name);
                    if (subTaskElements.length > 0 && !subTaskElement) { log(`- 未找到子任务Tab: "${subTaskConfig.name}"，跳过。`, 'warn'); continue; }

                    if (subTaskElement) { log(`- 切换到: "${subTaskConfig.name}"`, 'info'); robustClick(subTaskElement); await waitForSubTaskActive(subTaskConfig.name); }
                    else { log(`- 处理无Tab子任务: "${subTaskConfig.name}"`, 'info'); }

                    await executeActions(subTaskConfig.actions);
                }
            }

            log(`任务 "${currentMainTaskName}" 已按计划执行完毕。`, 'info');
            await GM_setValue('lastProcessedTaskName', currentMainTaskName); await GM_setValue('lastUrl', location.href);
            await navigateToNextMainTask();
        } catch (error) { log(`主流程错误: ${error.message}`, 'error'); console.error(error); toggleAutomation(); }
    ...
    async function executeActions(actions) {
        ...
        for (const action of actions) {
            if (!isAutomationRunning) { log('自动化已停止，中止动作序列。', 'warn'); return; }
            log(`-- 动作: ${action.type}` + (action.text ? ` (${action.text})` : ''));
            switch (action.type) {
                case 'video': await handleVideoTask(); break;
                case 'skip': await handleSkippableTask(); break;
                case 'vocabulary': await handleVocabularyTask(); break;
                case 'quiz': await doAnswer(); break;
                case 'click':
                    const success = await handleSpecificClick(action.text);
                    if (!success) { log(`点击动作失败，中止序列。`, 'error'); return; }
                    break;
                default: log(`未知动作类型: "${action.type}"`, 'warn');
            }
            await sleep(1500);
        }
    }

    async function handleSpecificClick(buttonText) {
        log(`正在查找按钮 [${buttonText}]...`, 'info');
        const selector = '.btn, button.ant-btn, .btn_learn_goon';
        const maxWaitTime = 7000; const interval = 500; let elapsedTime = 0;
        while (elapsedTime < maxWaitTime) {
            if (!isAutomationRunning) return false;
            const buttons = document.querySelectorAll(selector);
            for (const btn of buttons) {
                if (btn && btn.textContent.includes(buttonText) && !btn.disabled && btn.offsetParent !== null) {
                    btn.scrollIntoView({ behavior: 'smooth', block: 'center' }); await sleep(400);
                    log(`点击按钮: "${btn.textContent.trim()}"`, "success"); robustClick(btn);
                    await sleep(2000);
                    const confirmButton = document.querySelector('.ant-modal-confirm-btns .ant-btn-primary');
                    if (confirmButton && confirmButton.offsetParent !== null) { log("点击确认弹窗", 'info'); robustClick(confirmButton); }
                    return true;
                }
            }
            await sleep(interval); elapsedTime += interval;
        }
        log(`未找到可用的 [${buttonText}] 按钮。`, 'warn'); return false;
    }

    async function navigateToNextMainTask() {
        await sleep(2500); log("导航到下一个主任务...", 'info');
        ...
        if (currentIndex === -1) { log("无法定位当前主任务以进行导航。", 'error'); return; }
        const nextIndex = currentIndex + 1;
        if (nextIndex < allMainTasks.length) {
            const nextTaskElement = allMainTasks[nextIndex];
            const taskTitle = nextTaskElement.getAttribute('title') || nextTaskElement.textContent.trim();
            log(`点击: ${taskTitle}`); robustClick(nextTaskElement);
        } else { log("🎉 恭喜！课程已全部完成。", 'success'); toggleAutomation(); }
    }

    // --- Background Observers & Checkers ---
