// ==UserScript==
// @name         U校园全自动助手 (v23.0 - React终极版)
// @namespace    http://tampermonkey.net/
// @version      23.0.0
// @description  [React终极版] 破解万象！通过直接调用React内部方法，从根本上解决了所有填空题无法被识别的终极问题。这才是真正的、完全可靠的最终版本。
// @author       twj0
...
(function() {
    ...
    /**
     * [UPDATED] Sets the value of a React input element programmatically.
     * @param {HTMLInputElement} element The input or textarea element.
     * @param {string} value The value to set.
     */
    function setReactInputValue(element, value) {
        // Find the secret React property key
        const reactKey = Object.keys(element).find(key => key.startsWith('__reactProps$'));
        if (!reactKey) {
            log('无法找到React属性，使用传统方法。', 'warn');
            element.value = value;
            element.dispatchEvent(new Event('input', { bubbles: true }));
            return;
        }

        // Get the internal React component instance
        const reactInstance = element[reactKey];

        // This is the most reliable way to set the value for a controlled component
        if (reactInstance.onChange) {
            // Create a fake event object
            const event = {
                target: { value: value }
            };
            reactInstance.onChange(event);
        } else {
             log('未找到onChange处理器，使用传统方法。', 'warn');
            element.value = value;
            element.dispatchEvent(new Event('input', { bubbles: true }));
        }
    }
    ...
    async function doAnswer() {
        ...
        if (choiceQuestions.length > 0 && typeof answers === 'string' && /^[A-Z](?:\s*[A-Z])*$/.test(answers.trim())) {
            ...
            for (let i = 0; i < Math.min(choiceQuestions.length, answerLetters.length); i++) {
                ...
                for (const option of choiceQuestions[i].querySelectorAll('.option')) { if (option.querySelector('.caption')?.textContent.trim() === answerLetters[i]) { robustClick(option); await sleep(250); break; } }
            }
        ...
        if (fillInBlanks.length > 0 && typeof answers === 'string') {
            ...
            for (let i = 0; i < Math.min(fillInBlanks.length, answerLines.length); i++) {
                ...
                robustClick(blank); await sleep(200);
                ...
                if (targetWordElement) {
                    ...
                    log(`-- 未在词库中找到，通过React方法直接输入: "${answerText}"`, 'info');
                    setReactInputValue(blank, answerText); // Use the new React-safe method
                }
                await sleep(300);
            }
            log(`应用答案: ${activeSubTaskName}`, "success");
    ...
    const initInterval = setInterval(() => { if (document.body) { clearInterval(initInterval); createControlPanel(); startOptimizedModalObserver(); log('U校园助手(v23.0)已加载。'); } }, 100);
